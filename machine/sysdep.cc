...
//#else
//int creat(const char *, short unsigned int);
...
//int recvfrom(int, void *, unsigned int, unsigned int, struct sockaddr *, int *);
//int sendto(int, const void *, unsigned int, unsigned int, const struct sockaddr *, int);
...
//void abort();
//void exit();
//int mprotect(const void *, unsigned int, int);
...
AllocBoundedArray(int size)
{
    ...
    char *ptr = new(std::nothrow) char[pgSize * 2 + size];
