...
PendingInterrupt::PendingInterrupt(VoidFunctionPtr func, int param,  long long unsigned time, 
				IntType kind)
{
    handler = func;
    arg = param;
...
//----------------------------------------------------------------------
void
Interrupt::Schedule(VoidFunctionPtr handler, int arg, long long unsigned fromNow, IntType type)
{
    long long unsigned when = stats->totalTicks + fromNow;
    PendingInterrupt *toOccur = new PendingInterrupt(handler, arg, when, type);

    DEBUG('i', "Scheduling interrupt handler the %s at time = %llu\n", 
...
// Check if there is nothing more to do, and if so, quit
    ...
    DEBUG('i', "Invoking interrupt handler for the %s at time %llu\n", 
...
PrintPending(int arg)
{
    ...
    printf("Interrupt handler %s, scheduled at %llu\n", 
